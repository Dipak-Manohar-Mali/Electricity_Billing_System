        ...
        try {
            ...
            while(rs.next()) {
                ...
                labelname.setText(rs.getString("name"));
                    ...
                    ptmt=conn.prepareStatement("select * from bill where meter_no =? AND month = 'January'");
                    ptmt.setString(1, meter);
                    rs=ptmt.executeQuery();
...
//    }
    ...
    public void actionPerformed(ActionEvent ae) {
        if (ae.getSource() == pay) {
            try {
                Connection conn= ConectDb.connect();
                ptmt=conn.prepareStatement("UPDATE bill SET status='Paid' where meter_no =? AND month=?");
                                           ptmt.setString(1, meter);
					   ptmt.setString(2, cmonth.getSelectedItem());
